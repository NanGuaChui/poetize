<template>
  <div class="travel-container">
    ...
    <div class="travel-header my-animation-slide-top">
      <!-- 背景图片 -->
      <video class="index-video" autoplay="autoplay" muted="muted" loop="loop" :src="bgCover"></video>
      <div style="position: absolute; left: 20px; top: 20px">
        <!-- 标题 -->
        <div style="margin: 10px">
          <div>旅拍集</div>
          <div style="font-size: 36px; font-weight: bold; line-height: 1.5; margin-top: 20px">这里是我的旅拍哦</div>
        </div>
      </div>
      <div style="position: absolute; left: 20px; bottom: 40px; margin: 10px">每一张照片都是一次美好的记忆。</div>
    </div>
      ...
      <div class="photo-title-warp" v-if="!$common.isEmpty(photoTitleList)">
        ...
        >
          <proTag
            ...
            :color="$common.before_color_list[Math.floor(Math.random() * 6)]"
            style="margin: 12px"
          >
          </proTag>
        </div>
...
const { bgCover, photoTitleList, photoList } = useTravelInfoStore()
